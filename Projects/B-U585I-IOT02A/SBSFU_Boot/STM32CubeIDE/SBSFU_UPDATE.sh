...
$stm32programmercli $connect -d $SCRIPTPATH/../../BG96/Binary/sbsfu_s_init.bin $slot0 -v
ret=$?
if [ $ret != 0 ]; then
...
$stm32programmercli $connect -d $SCRIPTPATH/../../BG96/Binary/sbsfu_ns_init.bin $slot1 -v 
ret=$?
if [ $ret != 0 ]; then
  if [ "$1" != "AUTO" ]; then read -p "SBSFU_UPDATE script failed, press a key" -n1 -s; fi
  exit 1
...
$stm32programmercli $connect -d $SCRIPTPATH/../../BG96/Binary/sbsfu_init.bin $slot0 -v
ret=$?
if [ $ret != 0 ]; then
  if [ "$1" != "AUTO" ]; then read -p "SBSFU_UPDATE script failed, press a key" -n1 -s; fi
  exit 1
