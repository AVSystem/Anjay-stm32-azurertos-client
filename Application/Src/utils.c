...
void flash_aligned_writer_new(uint64_t *batch_buf,
                              size_t batch_buf_max_len_dwords,
                              size_t block_size_dwords,
                              flash_aligned_writer_cb_t *writer_cb,
                              flash_aligned_writer_t *out_writer) {
    assert(batch_buf);
    assert(batch_buf_max_len_dwords > 0);
    assert(block_size_dwords > 0);
    assert(batch_buf_max_len_dwords % block_size_dwords == 0);
    assert(writer_cb);
    assert(out_writer);

    out_writer->batch_buf = batch_buf;
    out_writer->batch_buf_max_len_bytes =
            sizeof(*out_writer->batch_buf) * batch_buf_max_len_dwords;
    out_writer->batch_buf_len_bytes = 0;
    out_writer->block_size_dwords = block_size_dwords;
...
int flash_aligned_writer_flush(flash_aligned_writer_t *writer) {
    ...
    const size_t block_size_bytes =
            writer->block_size_dwords * sizeof(*writer->batch_buf);
    const size_t tail_len_bytes =
            writer->batch_buf_len_bytes % block_size_bytes;

    // pad downloaded image if needed to fulfill flash driver's requirements
    if (tail_len_bytes != 0) {
        const size_t pad_len_bytes = block_size_bytes - tail_len_bytes;
        memset((uint8_t *) writer->batch_buf + writer->batch_buf_len_bytes,
               0xFF, pad_len_bytes);
        writer->batch_buf_len_bytes += pad_len_bytes;
    }
